...
extension SuggestionEngine {
    ...
    static func commitReason(
        ...
        // **A reason names a rule about the typed word, and the space bar inserts
        // whatever ranked first — so the two rules below have to check that those
        // are the same candidate.** Both are orthography: a dropped apostrophe and
        // a Hebrew final form, generated by `generatedCompletions` as
        // `Source.orthography`. `.personal` is a whole tier above that, so a
        // contact or a hand-typed entry that merely *extends* the keystrokes wins
        // the ranking, and a rule that answered on the typed letters alone armed
        // the space bar with a word it had asked no question about at all: with
        // `שלומית` in `supplementary`, typing `שלומ` committed the contact at
        // confidence 96, and the English half committed a contact `Dontrell` over
        // `dont` at 98. The contested guard above only catches the case where
        // *another* offered candidate also completes the keystrokes, so the plain
        // uncontested one went straight through it.
        //
        // **Refused rather than fallen through, because the fall-through is the
        // same defect one tier down.** Every question below this line is about a
        // candidate the cascade has not vouched for, and the four-letter gate at
        // the bottom would commit that same personal entry as `.unknownWord` —
        // `dont` is four letters no checker knows, so `Dontrell` would land again
        // at 87 or 68 instead of 98. The correction itself is still offered in the
        // bar and a deliberate tap still inserts it; only the space bar stops.
        func orthographyWins(_ correction: String) -> Bool {
        ...
        if typedLanguage.script == .hebrew, let finalForm = hebrewFinalFormCorrection(of: word),
            SeedLanguageModel.words(startingWith: word, in: typedLanguage, limit: 1).isEmpty
        {
            return orthographyWins(finalForm) ? .hebrewFinalForm : nil
        }
