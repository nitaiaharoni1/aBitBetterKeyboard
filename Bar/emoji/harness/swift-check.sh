...
# and `EmojiCatalog.swift` against the *real* `Resources/EmojiCatalog.json`,
# asks both sides the same 146 queries, and diffs the answers rank by rank.
#
...
# SwiftUI already are.
if grep -lE '^import (UIKit|SwiftUI)' "$core/EmojiSearch.swift" "$core/EmojiCatalog.swift"; then
    echo "" >&2
    echo "The files listed above import a platform framework." >&2
    echo "EmojiSearch.swift and EmojiCatalog.swift are the port-checkable core" >&2
    echo "and must import Foundation only. Put view code in EmojiPanel.swift or" >&2
    echo "EmojiSearchViews.swift instead." >&2
    exit 1
fi

cp "$core/EmojiSearch.swift" "$core/EmojiCatalog.swift" "$build/"
