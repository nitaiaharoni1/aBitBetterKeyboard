...
//
// It also reports whether the Apple Neural Engine frameworks are mapped into
// this process, because that is the difference between the deployments: the iOS
// Simulator maps none of them, macOS and a physical iPhone do. A simulator
// number is not a device number and this line is what proves which one you have.
//
...
/// Apple Neural Engine frameworks mapped into this process. The simulator maps
/// none; macOS and a physical iPhone map several. Vision's model deployment
/// follows this, which is why the two columns disagree by tens of megabytes.
...
for name in files {
    autoreleasepool {
        let image = scaled(loadImage(directory.appendingPathComponent(name)), by: scale)
        if firstSize.isEmpty { firstSize = "\(image.width)x\(image.height)" }
        runner.run(image)
    }
    let now = footprintMB()
    peak = max(peak, now)
    rows.append((name, now))
}
let settle = autoreleasepool { footprintMB() }
peak = max(peak, settle)

let ane = aneFrameworks()
let size = firstSize
print("deployment   \(deployment)")
print("ane-mapped   \(ane.isEmpty ? "none" : ane.joined(separator: " "))")
print("config       \(config.rawValue)  scale=\(scale)  first-image=\(size)  images=\(files.count)")
print(String(format: "base         %.1f MB", base))
print(String(format: "peak         %.1f MB", peak))
print(String(format: "final        %.1f MB   (delta %+.1f MB)", settle, settle - base))
for (name, value) in rows { print(String(format: "  %-10s %.1f", (name as NSString).utf8String!, value)) }
