...
- **`Bar/emoji/harness/swift-check.sh` compiles the real `EmojiSearch.swift`,
  ...
  broken.
- **Emoji glyphs are cached for the life of the process, and nothing can release them.** Drawing all 1,870 catalogue emoji once grew a macOS process by 12 MB that stayed through a second pass, a released font and three seconds idle, at 20 pt and 30 pt alike, so it is a per-glyph cost and a smaller glyph does not buy it back. In a keyboard with roughly 50 MB to live in that is a quarter of the budget, and the grid was the one surface that never checked headroom. `KeyboardController.reserveMemoryForEmojiGlyphs()` runs when the grid opens and when a scroll reaches a new category: below `emojiGlyphReserveMB` (16) it stops suggestion work, which drops the rebuildable word lists and, through `onMemoryPressure`, cancels a cache warm still in flight so it cannot rebuild them. Suggestions come back on the next appearance with headroom. The iPhone figure is not measured; the macOS one is the reason for the number.
