...
- `Bar/` holds the frozen corpora and scoring harnesses (`ai-text`, `dictation`, `screen-context`, `layouts`, `typing`, `grouped`). `Scripts/prove-*.sh` hold the checks that fail the build when an architectural invariant is broken. **`grouped` is the only one that measures something this repo does not ship** — a keyboard whose keys each carry several letters — so it needs no simulator and runs in seconds; see `.claude/docs/grouped-keys-design.md`. Its picker was removed from the Keys tab (a measured top-1 accuracy reads to a buyer as "wrong word one in ten"), while the decoder, the lexicon and the corpus stayed, because they are what a glide-typing feature would reuse. **`Bar/drift/` re-runs those corpora unattended and appends one dated record per run**, because every absolute number in `README.md` is a reading with a date on it and the model behind two of them is a moving alias with no version to pin to; it defaults to the free set, needs `--paid` for the corpora that spend model calls, and fires on a trend across runs rather than on one delta — `Bar/drift/README.md` has the arithmetic.

## What is real

**Two finished features are deliberately not in the v1 build, and both are flags rather than deletions.** `FeatureFlags.screenCaptureReply` (in `AIKeyboardShared`) is `false`: the whole ReplayKit path is gated off, because no line of it has ever executed anywhere and it asked the user for a screen recording on top of Full Access. **Reply is sourced from the pasteboard instead** (`ReplySource`, the CopyClip ledger: copy the message, tap Paste, tap Reply). `AppFeatureFlags.subscriptionPaywall` is `false` for the same class of reason, until real StoreKit lands. Each flag names its own condition for flipping; do not flip one because the code compiles. `.claude/docs/screen-capture-v1-hold.md` is the decision record. `MockScreenContext`'s scripted demo survives as a test fixture with no call site outside the test bundle. `MockDictation`, `MockAI` and `MockSuggestionEngine` are all deleted — `DictationService` records in the containing app, `SpeechGate` decides on the device whether anybody spoke and `CloudDictation` transcribes; `RoutedIntelligence` picks between `FoundationModelsEngine` and `CloudIntelligence`; and `SuggestionEngine` runs on `UITextChecker` and `UILexicon`. `README.md` carries the table of what is measured and what is only compiled.
...
- **`.none` in an optional position is `Optional.none`, and this repo has now been bitten by it four times.** When the expression's type is `T?` and `T` has a case named `none`, Swift resolves `.none` to nil and says nothing. `FrameIdentity` is spelled `.absent` for this reason; on top of that, `target.autocapitalizationType = .none` set nil rather than "never capitalise" and failed two tests against correct code, and `XCTAssertEqual(block?.remedy, .none)` asserted "no remedy at all" against a block correctly carrying `Remedy.none`. Write the type out: `UITextAutocapitalizationType.none`. See `.claude/docs/test-suite-state.md`.
...
- **Do not drive the simulator to verify.** No clicks, `simctl launch` / `terminate` / `io`, or XCUITest you started. That steals the mouse. Build, then stop. I look.
...
| Emoji grid and catalogue | A skin tone you can spell is not one that exists, and the popup a scroll view holds cannot be a plain gesture. | `.claude/rules/emoji.md` |
...
| Tests | `.claude/docs/testing.md`, and `.claude/docs/test-suite-state.md` for what the suite currently reports and which failures were already there |
...
| Anything behind `FeatureFlags.screenCaptureReply` | `.claude/docs/screen-capture-v1-hold.md` — what was held back, why, and the exact condition for turning it on |
| Dictation entry points | `.claude/docs/dictation-front-door.md` — the App Intent, and why a Control Center control cannot start a microphone |
| Analytics | `.claude/docs/analytics-policy.md` — the six events, the never-list, and why the extensions send nothing |
| Testing on a phone | `.claude/docs/device-test-checklist.md` — the checks a Mac cannot answer, in the order that wastes least time |
