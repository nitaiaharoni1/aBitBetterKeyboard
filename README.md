...
| Mock | Real thing |
|---|---|
| ~~`MockSuggestionEngine`~~ — now `SuggestionEngine` | **Done, with one part honestly left out.** Completions, spelling guesses and autocorrect all come from `UITextChecker`, the on-device engine the system keyboard's own autocorrect draws on, plus the user's own names and text replacements via `UILexicon`. **Hebrew works here**, which makes this the one Apple text API in this project that has it: `UITextChecker.availableLanguages` lists `he_IL` among 42, `אנ` completes to `אני`, and `שלומ` is corrected to `שלום` — pinned end to end by `SuggestionEngineTests`. What is *not* real is predicting the next word from nothing typed: that is QuickType, and Apple ships no public API for it, so it remains a small fixed table and says so where it lives rather than being dressed up. |
